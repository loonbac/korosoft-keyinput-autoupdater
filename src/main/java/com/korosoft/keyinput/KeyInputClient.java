...
public class KeyInputClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // of this throwing.
        org.slf4j.Logger authLog = org.slf4j.LoggerFactory.getLogger("keyinput/koroauth");
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModUpdatePayload.ID, (payload, context) ->
                context.client().execute(() -> {
                    if (ModUpdater.get().isApplying()) {
                        // Server re-pushed because the player disconnected without applying.
                        // Just reinstall the overlay; do NOT start a second download.
                        MinecraftClient mc = MinecraftClient.getInstance();
                        mc.setScreen(new ModUpdateScreen(
                                "Actualizando KeyInput",
                                payload.message(),
                                mc.currentScreen));
                        return;
                    }
